...
#define AUTO_WATERING_COOLDOWN 60000  // 1 minute between auto-watering cycles
    ...
    if (millis() - lastHistoryUpload >= HISTORY_UPLOAD_INTERVAL) {
        ...
        if (Firebase.RTDB.getJSON(&fbdo, historyPath)) {
            ...
            for (size_t i = 0; i < count; i++) {
                ...
                String path = String(historyPath) + String("/") + keyStr;
                historyJson.get(entry, path + String("/timestamp"));
